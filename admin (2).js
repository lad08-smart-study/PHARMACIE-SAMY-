...
async function handleLogin() {
    ...
    if (error) {
        if (loginMessage) {
            loginMessage.innerHTML = `<div class="alert alert-error">${error.message || 'Erreur de connexion'}</div>`;
        }
    }
...
async function loadAdminProducts() {
    ...
    if (!client) {
        tbody.innerHTML = '<tr><td colspan="7" class="text-center alert alert-error">Supabase not initialized</td></tr>';
        return;
    }
    ...
    if (error) {
        tbody.innerHTML = `<tr><td colspan="7" class="text-center alert alert-error">${error.message}</td></tr>`;
        return;
    }
...
async function loadAdminCategories() {
    ...
    const client = getSupabase();
    if (!client) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center alert alert-error">Supabase not initialized</td></tr>';
        return;
    }
...
async function loadAdminBrands() {
    ...
    const client = getSupabase();
    if (!client) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center alert alert-error">Supabase not initialized</td></tr>';
        return;
    }
